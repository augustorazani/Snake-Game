...
namespace Snake_Game
{
    internal class Snake
    {
        ...
        public Snake()
        { //construtor para armazenar a posição máxima que a cobra poderá ocupar
            Location = new Point[28 * 28];
            Reset();
        }

        public void Reset()
        { //determinar a posição inicial sempre que iniciar um novo jogo
            Length = 5; //comprimento inicial de 5 quadrados (pode ser alterado)
            for (int i = 0; i < Length; i++)
            { //loop para setar a posição inicial
                Location[i].X = 12;
                Location[i].Y = 12;
            }
        }
        ...
        public void Follow()
        {//método follow para o corpo da cobra seguir a cabeça e não separar
